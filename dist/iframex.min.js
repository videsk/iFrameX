...
this.body.appendChild(iframe);this.iframe=document.querySelector(`[iframe-id="${this.id}"]`);this._addEventListener();(typeof callback==='function')&&callback(iframe)}else{console.error('An iframe already exists, please instance a new iFrameX. Read docs here: https://github.com/matiaslopezd/iframex');(typeof callback==='function')&&callback(null)}};_addElements(elements){function _createAndBind(i){const el=document.createElement(i.type);if(i.content&&i.content!==''){if(i.type==='script'||i.type==='style'){el.appendChild(document.createTextNode(i.content))}else{el.innerHTML=i.content}};(i.attr)&&Object.keys(i.attr).map((a)=>{el.setAttribute(a,i.attr[a])});return el}
if(Array.isArray(elements)&&elements.length>0){elements.map((obj)=>{let el=(!obj.append)?'body':obj.append;setTimeout(()=>this.iframe.contentWindow.document.querySelector(el).appendChild(_createAndBind(obj)),1)})}else if(typeof elements==='object'){let el=(!elements.append)?'body':elements.append;setTimeout(()=>this.iframe.contentWindow.document.querySelector(el).appendChild(_createAndBind(elements)),1)}};_addEventListener(){this._addElements({type:'script',content:this._bindEvent('window',`'message'`,this.action)})};_bindEvent(element,eventName,eventHandler){let event=null;element=(element==='window')?element:`document.querySelector(${element})`;if(window.addEventListener){event=`${element}.addEventListener(${eventName}, ${eventHandler}, false);`}else if(window.attachEvent){event=`${element}.attachEvent(on${eventName}, ${eventHandler});`}
return event}}